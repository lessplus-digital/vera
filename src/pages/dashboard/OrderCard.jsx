...
export default function OrderCard({ order, isNew, onUpdated }) {
  ...
  const metodo = METODO_LABEL[order.metodo_pago] || { icon: '💳', cls: 'blue' }
      ...
      <div className={`order-card${isNew ? ' is-new' : ''}`}>
        ...
        <div className="head">
          ...
          <div className="total-col">
            ...
            <div className={`method ${metodo.cls}`}>{metodo.icon} {order.metodo_pago}</div>
          </div>
        </div>

        {/* Tipo pedido + estado pago */}
        <div className="tags">
          <span className={`badge ${order.tipo_pedido === 'domicilio' ? 'purple' : 'blue'}`}>
        ...
        {/* Notas */}
        {order.notas && <div className="notes">📝 {order.notas}</div>}

        {/* Esperando comprobante */}
        {order.metodo_pago === 'Transferencia' && !order.comprobante_url && order.estado === 'pendiente' && (
          <div className="transfer-wait">⏳ Esperando comprobante de transferencia</div>
        )}

        {/* Ver comprobante */}
        {order.comprobante_url && (
          <button className="oc-btn voucher" onClick={() => setShowComprobante(true)}>
        ...
        {order.estado === 'pendiente' && (
          <button className="oc-btn edit" onClick={() => setShowEdit(true)}>
            ✏️ Editar pedido
          </button>
        )}
          ...
          <div className="voucher-overlay" onClick={() => setShowComprobante(false)}>
            <div className="panel" onClick={e => e.stopPropagation()}>
              <div className="vp-head">
                ...
                <button className="close-btn" onClick={() => setShowComprobante(false)}>✕</button>
              </div>
